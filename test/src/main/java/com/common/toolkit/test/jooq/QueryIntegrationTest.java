...
import static com.common.toolkit.database.jooq.db.public_.tables.Author.AUTHOR;
import static com.common.toolkit.database.jooq.db.public_.tables.AuthorBook.AUTHOR_BOOK;
import static com.common.toolkit.database.jooq.db.public_.tables.Book.BOOK;
...
public class QueryIntegrationTest {
   ...
   */
  @Test
  public void jdbcTemplate_jooq() {
    final ResultQuery<Record3<Integer, String, Integer>> query = dsl
        .select(AUTHOR.ID, AUTHOR.LAST_NAME, DSL
            .count())
        .from(AUTHOR)
        .join(AUTHOR_BOOK).on(AUTHOR.ID.equal(AUTHOR_BOOK.AUTHOR_ID))
        .join(BOOK).on(AUTHOR_BOOK.BOOK_ID.equal(BOOK.ID))
        .groupBy(AUTHOR.LAST_NAME);
    List<AuthorAndBooks> result = jdbcTemplate.query(
        query.getSQL(),
    ...
    InsertSetMoreStep<AuthorRecord> insert = dsl.insertInto(AUTHOR)
        .set(AUTHOR.ID, 4)
        .set(AUTHOR.FIRST_NAME, "Herbert")
        .set(AUTHOR.LAST_NAME, "Schildt");
    //插入
    jdbcTemplate.execute(insert.getSQL());
  ...
  public void givenValidData_whenInserting_thenSucceed() {
    ...
    final Result<Record3<Integer, String, Integer>> result = dsl
        .select(AUTHOR.ID, AUTHOR.LAST_NAME, DSL
            .count())
        .from(AUTHOR)
        .join(AUTHOR_BOOK).on(AUTHOR.ID.equal(AUTHOR_BOOK.AUTHOR_ID))
  ...
  public void givenValidData_whenUpdating_thenSucceed() {
    ...
    final Result<Record3<Integer, String, String>> result = dsl
